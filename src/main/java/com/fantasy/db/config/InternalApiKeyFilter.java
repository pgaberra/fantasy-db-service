...
public class InternalApiKeyFilter extends OncePerRequestFilter {
    ...
    // Fail closed: this service returns password hashes, so it must never serve an unauthenticated
    // internal API. If no key is configured, refuse to start rather than silently allowing all
    // callers. Run any environment (local included) with INTERNAL_API_KEY set; tests provide one.
    @PostConstruct
    void requireApiKey() {
    ...
    protected boolean shouldNotFilter(HttpServletRequest request) {
        return isActuatorHealthOrInfo(request);
    }

    // Match the decoded + normalised path (not the raw URI) and pin the exemption to the health
    // and info probes only, so a traversal such as /actuator/health/../../api/v1/users cannot
    // slip a protected path past the filter under an /actuator prefix.
    private static boolean isActuatorHealthOrInfo(HttpServletRequest request) {
        String path;
        try {
            ...
            // A malformed %-sequence isn't a health probe — fall through to the key check (401),
